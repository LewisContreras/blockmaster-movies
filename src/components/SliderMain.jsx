...
function SliderMain() {
    ...
    return (
        ...
        >
        {sliderImages.map((imgInfo,index)=><SwiperSlide  key={index}>
        <Box height={["200px","200px","360px"]} position="relative">
                  <Img borderRadius="10px" width="100%" height="100%" src={imgInfo.url} alt={imgInfo.alt} />
                  <HStack position="absolute" bottom="20px" left="20px">
                    <Button leftIcon={<FaPlay/>} color="brand.black" bgColor="brand.primary" onClick={()=>dispatch(movieTrailer(`https://res.cloudinary.com/dfx4uggfe/video/upload/${imgInfo.moviePath}`))} >VER AHORA</Button>
